...
import com.hkbufyp.hrms.ui.screen.management.attendance.clockin_devices.wifi_devices.RemoveWifiDeviceConfirmationBox
import com.hkbufyp.hrms.ui.screen.management.attendance.clockin_devices.wifi_devices.WifiDeviceEvent
...
fun BleDeviceScreen(
    ...
    val uiState by bleDeviceViewModel.uiState.collectAsState()

    LaunchedEffect(true) {
        sharedViewModel.isShowAppBar(false)
        bleDeviceViewModel.onEvent(BleDeviceEvent.Enter)

        bleDeviceViewModel.isShowSnackbar.collectLatest {
            if (it) {
    ...
    Scaffold(
        ...
        Box(
            ...
            if (uiState.isLoading) {
                ...
                FullScreenLoading(
                    show = uiState.isRemoving
                )

                RemoveDeviceConfirmationBox(
                    showDialog = uiState.isShowConfirmationBox,
                    device = uiState.removeDevice,
                    onConfirm = {
                        bleDeviceViewModel.onEvent(BleDeviceEvent.Confirm)
                    },
                    onDismiss = {
                        bleDeviceViewModel.onEvent(BleDeviceEvent.Cancel)
                    }
                )
                                ...
                                Card(
                                    ...
                                    Box (
                                        ...
                                        Row (
                                            ...
                                            IconButton(onClick = {
                                                bleDeviceViewModel.onEvent(BleDeviceEvent.RemoveDevice(device))
                                            }) {
...
@Composable
fun RemoveDeviceConfirmationBox(
    showDialog: Boolean,
    device: BleDevice?,
    onConfirm: () -> Unit,
    onDismiss: () -> Unit
    ...
    if (showDialog && device != null) {
        AlertDialog(
            ...
            text = {
                Column {
                    Text(text = "Are you sure to remove this ble for clock-in verification?")
                    Text(text = "Name: ${device.name}")
                    Text(text = "Mac: ${device.mac}")
                }
            }
        )
    }
